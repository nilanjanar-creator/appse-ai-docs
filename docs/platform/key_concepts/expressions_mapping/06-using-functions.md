...
| `date_diff()` | `{{ date_diff($payload.updated_at, $payload.created_at, 'days') }}` | `18` | Date Difference
...
```js
{
  ...
  "U_DaysDifference": "{{ date_diff($payload.updated_at, $payload.created_at, 'hours') }}"
}
```
...
```js
{
  ...
  "U_DaysDifference": 432.4352777777778
}
```
